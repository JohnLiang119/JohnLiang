...
提供跨電腦開發環境之一鍵初始設置腳本（`setup_workspace.ps1`）與日常雙向更新拉取腳本（`pull_all.ps1`），實現專案原始碼與工作區設定的高效同步。

## ADDED Requirements

### Requirement: Workspace Initial Setup Script Execution
系統必須在 `C:\JohnLiang` 根目錄提供 PowerShell 腳本 `setup_workspace.ps1`。當執行時，檢查 `..Project/avd` 是否存在；若不存在則自動自 GitHub (`https://github.com/JohnLiang119/avd.git`) clone 下來，並自動觸發 `restore_avd.ps1` 還原依賴。

#### Scenario: Setup workspace on a clean machine
- **WHEN** 使用者在新電腦執行 `setup_workspace.ps1`
